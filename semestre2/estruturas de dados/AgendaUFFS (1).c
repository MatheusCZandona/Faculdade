...
Evento *newElemento(int codigo, int dia, int mes, int ano, int hora, int minuto, float duracao, const char *descricao) {
    ...
    new->descricao[sizeof(new->descricao) - 1] = '\0';
    new->next = NULL;
    return new;
}
...
void calcularFimEvento(Data *inicio, float duracao, Data *fim) {
    *fim = *inicio;
    ...
    fim->minuto += duracaoMinutos % 60; // calcula os minutos que sobram das horas
    fim->hora += duracaoMinutos / 60 + fim->minuto / 60; //aumenta a hora se os minutos excedem 60
    fim->minuto %= 60; // garante que não excede 60 min

    //para o dia seguinte (considera meses com 30 dias)
    if (fim->hora >= 24) {
        fim->hora -= 24;
        fim->dia++;
        if (fim->dia > 30) {  // Meses são tratados como tendo 30 dias
            fim->dia = 1;
            fim->mes++;
            if (fim->mes > 12) {
                fim->mes = 1;
...
bool adicionarEvento(Evento **lista, Evento *novo) {
    ...
    if ((anterior != NULL && conflito(anterior, novo)) || 
        ...
        printf("Conflito de horário com o evento %d: %s\n", (anterior ? anterior->codigo : atual->codigo), (anterior ? anterior->descricao : atual->descricao));
        return false;
    }
...
void consultarEvento(Evento *lista, int dia, int hora, int mes, int ano) {
    Evento *aux = lista;
    int cont = 0;
    ...
    while (aux != NULL) {
        ...
        } else if (aux->dataEvento.dia > dia) {
            break;
        } else {
            if (aux->dataEvento.dia == dia && aux->dataEvento.hora == hora && aux->dataEvento.mes == mes && aux->dataEvento.ano == ano) {
                printf("Evento %d: %s\n", aux->codigo, aux->descricao);
                cont++;
                break;
            }
...
void alterarEvento(Evento *lista, float duracao, char descricao[100], int dia, int hora) {
    Evento *aux = lista;
...
void printList(Evento *lista) {
    ...
    while (aux != NULL) {
        printf("Evento %d: %s. Duração: %.2f horas\n", aux->codigo, aux->descricao, aux->duracao);
...
int main(void) {
    ...
    while (1) {
      ...
      if (consulta == 1) {
        do {
          printf("Digite a data do evento (dia, mês e ano): ");
          scanf("%d %d %d", &dia, &mes, &ano);
        } while (dia <= 0 || dia > 31 || mes <= 0 || mes > 12 || ano < 0);

        consultarEvento(head, dia, -1, mes, ano);
      } else if (consulta == 2) {
        do {
          printf("Digite a data do evento (dia, mês e ano): ");
          scanf("%d %d %d", &dia, &mes, &ano);
        ...
        consultarEvento(head, dia, hora, mes, ano);
      }
      printf("\n");
            break;
